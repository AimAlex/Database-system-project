...
public class BTreeFile implements DbFile {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        while(iter.hasNext()){
            Tuple tt = iter.next();
            page.deleteTuple(tt);
            newPage.insertTuple(tt);
        }

        newPage.setRightSiblingId(page.getRightSiblingId());
        newPage.setLeftSiblingId(page.getId());
        if(newPage.getRightSiblingId() != null){
            ((BTreeLeafPage) getPage(tid, dirtypages, newPage.getRightSiblingId(), Permissions.READ_WRITE)).setLeftSiblingId(newPage.getId());
        }
        page.setRightSiblingId(newPage.getId());

        BTreeInternalPage parPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), tmp.getField(keyField));
        BTreeEntry entry = new BTreeEntry(tmp.getField(keyField), page.getId(), newPage.getId());
        parPage.insertEntry(entry);
        updateParentPointer(tid, dirtypages, parPage.getId(), page.getId());
        updateParentPointer(tid, dirtypages, parPage.getId(), newPage.getId());
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
	    ...
	    newPage.insertEntry(tmp);

	    while(iter.hasNext()){
	        BTreeEntry tt = iter.next();
	        page.deleteKeyAndRightChild(tt);
	        newPage.insertEntry(tt);
        }

        newPage.deleteKeyAndLeftChild(tmp);

	    BTreeInternalPage parPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), tmp.getKey());
	    BTreeEntry entry = new BTreeEntry(tmp.getKey(), page.getId(), newPage.getId());
	    parPage.insertEntry(entry);
	    updateParentPointers(tid, dirtypages, newPage);
        updateParentPointers(tid, dirtypages, parPage);

	    if(field.compare(Op.LESS_THAN_OR_EQ, tmp.getKey())){
	        return page;
        }
        else{
	        return newPage;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
            ...
            int transNum = (sibling.getNumTuples() - page.getNumTuples()) / 2;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
	    ...
	    int transNum = (leftSibling.getNumEntries() - page.getNumEntries() - 1) / 2;

	    Iterator<BTreeEntry> iter = leftSibling.reverseIterator();

	    BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), iter.next().getRightChild(), page.iterator().next().getLeftChild());

	    page.insertEntry(newEntry);

        iter = leftSibling.reverseIterator();
        ...
        BTreeEntry parNewEntry = iter.next();
        leftSibling.deleteKeyAndRightChild(parNewEntry);
        parentEntry.setKey(parNewEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        int transNum = (rightSibling.getNumEntries() - page.getNumEntries() - 1) / 2;

        Iterator<BTreeEntry> iter = rightSibling.iterator();

        BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), iter.next().getLeftChild());

        page.insertEntry(newEntry);

        iter = rightSibling.iterator();
        ...
        BTreeEntry parNewEntry = iter.next();
        rightSibling.deleteKeyAndLeftChild(parNewEntry);
        parentEntry.setKey(parNewEntry.getKey());
        parent.updateEntry(parentEntry);
        updateParentPointers(tid, dirtypages, page);
		// some code goes here
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        BTreePageId newRightSibling = rightPage.getRightSiblingId();
        leftPage.setRightSiblingId(newRightSibling);
        if (newRightSibling != null){
            ((BTreeLeafPage)getPage(tid, dirtypages, newRightSibling, Permissions.READ_WRITE)).setLeftSiblingId(leftPage.getId());
        }
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
	    ...
	    BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), iter.next().getLeftChild());
	    leftPage.insertEntry(newEntry);
	    iter = rightPage.iterator();
